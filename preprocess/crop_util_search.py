...
def crop_rotate_part(image, bbox,eye_left, eye_right, bb_width, is_gray=True,
                   dest_width=85, extend=0.3, dest_sz=(96, 112)):
    """
    this function is used to do similarity transformation,
    put the eye into horizontal direction
    ...
    top = max(0,int(mid_eye[1]-bbox.height()))
    bottom = min(image.shape[0]-1,int(mid_eye[1]+bbox.height()))
    left = max(0,int(mid_eye[0] - bbox.width()))
    right = min(image.shape[1]-1,int(mid_eye[0] + bbox.width()))
    ...
    save_croped = crop_image.copy()
    # mid_eye = (float(bbox.width()),float(bbox.height()))
    mid_eye = (float(mid_eye[0]-left),float(mid_eye[1]-top))
