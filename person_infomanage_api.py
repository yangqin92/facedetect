...
# 增加和删除操作触发人脸图片初始化，修改对图片进行判断再决定是否替换特征值
# 遗留问题：appid的验证需要在商定
                ...
                if result == ():
                    if dict1["data"]["name"]:
                        ...
                        if dict1["data"]["image"]:
                            ...
                            tag = dict1["data"]["tag"]
                            code = newperson_tables(p_id, name, fileout,tag)
                            if code == 0:
                                dict_success = {"data":{"p_id":"{}".format(p_id)},"code": 0, "msg": "success"}
                                 # 重新初始化人脸信息
                                ...
                                dict_error = {"code": 1, "msg": "添加失败请检查数据库是否连接！"}
                                return json.dumps(dict_error, ensure_ascii=False)
                        else:
                            dict_error = {"code": 1, "msg": "请给出image参数！"}
                            return json.dumps(dict_error, ensure_ascii=False)
                    else:
                        dict_error = {"code": 1, "msg": "请给出name参数！"}
                        return json.dumps(dict_error, ensure_ascii=False)
                else:
                    dict_error = {"code": 1, "msg": "该号码已经注册！"}
                    return json.dumps(dict_error, ensure_ascii=False)
            else:
                dict_error = {"code": 1, "msg": "手机号码错误！"}
                return json.dumps(dict_error, ensure_ascii=False)
        else:
            dict_error = {"code": 1, "msg": "验证无效！"}
            return json.dumps(dict_error, ensure_ascii=False)
    else:
        dict_error = {"code":1,"msg":"只接受post请求！"}
        return json.dumps(dict_error, ensure_ascii=False)
...
def delperson():
        if request.method == 'POST':
            ...
            if dict1["appid"] == "ZQKJ2018":
                if len(dict1["data"]["p_id"]) == 11:
                    p_id = dict1["data"]["p_id"]
                    image_url = "./user_pic/{}.bmp".format(p_id)
                    if os.path.exists(image_url):
                        os.remove(image_url)
                    else:
                        dict_error = {"code": 1, "msg": "image文件不存在！"}
                        return json.dumps(dict_error, ensure_ascii=False)
                    init_fea.extract_feature_enter()
                    ...
                    if code == 0:
                        ...
                        dict_error = {"code": 1, "msg": "删除失败请检查数据库是否连接！"}
                        return json.dumps(dict_error, ensure_ascii=False)
                else:
                    dict_error = {"code": 1, "msg": "手机号码错误！"}
                    return json.dumps(dict_error, ensure_ascii=False)
...
@app.route('/face/setperson' , methods=['GET', 'POST'])#更新人脸信息不改手机号,可以改name，对图片先比较再决定是否替换特征值，如果改手机号选择先删除再增加
def setperson():
    new = 0
    if request.method == 'POST':
        a = request.get_data()
            ...
            if len(dict1["data"]["p_id"]) == 11:
                ...
                if dict1["data"]["name"]:
                    ...
                    if dict1["data"]["image"]:
                        tag = dict1["data"]["tag"]
                        image_url = dict1["data"]["image"]
                        fileout = './card/{}.bmp'.format(p_id) #先保存在card下
                        urllib.request.urlretrieve(image_url, fileout)
                        ...
                        code = setperson_tables(p_id, name, image_first,tag)
                        if code == 0:

                            dict_success = {"data":{"p_id":"{}".format(p_id)},"code": 0, "msg": "success"}
                            return json.dumps(dict_success, ensure_ascii=False)
                        else:
                            dict_error = {"code": 1, "msg": "更新失败请检查数据库是否连接！"}
                            return json.dumps(dict_error, ensure_ascii=False)
                    else:
                        dict_error = {"code": 1, "msg": "请给出image参数！"}
                        return json.dumps(dict_error, ensure_ascii=False)
            ...
            if len(dict1["data"]["p_id"]) == 11:
                ...
                if results != 1:
                    if results == ():
                        ...
                        dict_success = {"data": {"p_id": "{}".format(p_id), "name": "{}".format(name), "image": image, "tag":tag}, "code": 0, "msg": "success"}
                        return json.dumps(dict_success, ensure_ascii=False)

                else:
                    dict_error = {"code": 1, "msg": "获取失败请检查数据库是否连接！"}
                    return json.dumps(dict_error, ensure_ascii=False)
        ...
        if dict1["appid"] == "ZQKJ2018":
            if dict1["data"]["image"]:
                ...
                if p_id_list == [] and api_unknum != 0:
                    dict_success = {"code": 0, "msg": "success", "data": {"已知访客": 0,
...
def count():
    if request.method == 'POST':
        ...
        if dict1["appid"] == "ZQKJ2018":
            ...
            if count != 1:
                dict_success = {"code": 0, "msg": "success", "data": {"count": "{}".format(count)}}
                return json.dumps(dict_success, ensure_ascii=False)
            else:
                dict_error = {"code": 1, "msg": "获取失败请检查数据库是否连接！"}
                return json.dumps(dict_error, ensure_ascii=False)
...
@app.route('/user_pic/<p_id>', methods=['GET','POST'])
def index(p_id):
    image = open ('./user_pic/{}.bmp'.format(p_id))
    #resp = Response(image, mimetype="image/bmp")
    #cv2.imshow("frame",image)
    resp = make_response(image)
    resp.mimetype = "image/bmp"
    return resp
